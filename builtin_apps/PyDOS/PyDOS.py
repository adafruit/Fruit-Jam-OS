...
def PyDOS():
            ...
            if ans[:1].upper() == "R":
                ...
                if input("Are you sure? (Y/N): ").upper() == "Y":
                    ...
                    try:
                        ...
                        print(
                            "\nThis microcontroller doesn't appear to have a CPSAVES drive. To"
                        )
                        print(
                            "switch to read-write mode you will need to edit the boot.py file from"
                        )
                        print(
                            "a connected host computer and then powercycle the microcontroller.\n"
                        )
                        print("Add the following line to the end of the /boot.py file:")
                        print("storage.remount('/',False)\n")
                        print(
                            "To return to readonly mode you can use the PyDOS readonly command"
                        )
                        print("or edit the /boot.py file using the Fruit Jam Editor or")
                        print("the PyDOS edlin line editor.\n")
            elif ans[:1].upper() != "C":
                print("\nInvalid entry", ans[:1])
    ...
    def exCmd(cFile, passedIn):
        try:
            ...
            print_exception(
                err, err, err.__traceback__ if hasattr(err, "__traceback__") else None
            )
            envVars["lasterror"] = format_exception(
                err, err, err.__traceback__ if hasattr(err, "__traceback__") else None
            )
        ...
        if not quit:
            ...
            if swRecur:
                for _dir in sorted(os.listdir(dPath), key=str.upper):
                    ...
                    if not aFile(dStat):
                        try:
                            (nLines, nFiles, tFSize, nDirs, quit) = dirLoop(
                                dStat,
                                (dirPat if dirPat is not None else ""),
                                isFile,
                                swPause,
                                swWide,
                                swRecur,
            ...
            if prSum and not quit:
                ...
                (quit, nLines) = scrnPause(
                    ...
                    [
                        (
                            f"{' ' * (4 - len(str(nFiles)))} {nFiles} File(s){' ' * (32 - len(str(tFSize)) - scrAdj1)} {tFSize} Bytes."
                        )[:scrWdth],
                        (
                            f"{' ' * (4 - len(str(nDirs)))} {nDirs} Dir(s){' ' * (33 - len(str(availDisk)) - scrAdj1)} {availDisk} Bytes free."
                        )[:scrWdth],
                        "",
                    ],
                    "",
    ...
    def prDir(dirPath, swBits):
        ...
        if validPath:
            ...
            if (
                lastDir in os.listdir()
                or lastDir in ".."
                or "*" in lastDir
                or "?" in lastDir
                or swBits & int("000010", 2)
            ):
                ...
                dirLoop(
                    ...
                    lastDir,
                    isFile,
                    bool(swBits & int("000100", 2)),
                    bool(swBits & int("010000", 2)),
                    bool(swBits & int("000010", 2)),
                    True,
                )
            else:
                    ...
                    if aFile(Dir + _dir):
                        ...
                        if removDirs:
                            ...
                            try:
                                ...
                                print(
                                    f"Unable to remove: {Dir}{_dir}, Exception: {err}"
                                )
                                break
                else:
                    print(
                        ...
                        + Dir
                        + _dir
                        + ". Filename too long for wildcard operation."
                    )
    ...
    def readBATFile(BATfile):
        ...
        for batLine in BATfile:
            ...
            if batLine.strip()[:1] == ":" and len(batLine.strip().split(" ")[0]) > 1:
                batLabels[batLine.strip().split(" ")[0][1:]] = [
                    batLineNo,
                    batIndex[batLineNo],
                ]
        BATfile.seek(0)
    ...
    if "autoexec.bat" in ",".join(os.listdir()).lower().split(","):
        ...
        BATfile = open(
            os.listdir()[
                (",".join(os.listdir()).lower().split(",")).index("autoexec.bat")
            ]
        )
    ...
    while True:
        ...
        if cmd == "" or cmd == "REM":
            ...
            print(
                "File Commands: DIR[/p][/w][/s], RENAME, DEL[/s], TYPE[/p], CD, MKDIR, RMDIR[/s], COPY[/y]"
            )
            print("Environment Commands: HELP, SET[/p][/a], PROMPT, PATH, READONLY")
            print(
                "Operating System Commands: EXIT, VER, MEM, DATE [mm-dd-yy], TIME [hh:mm:ss]"
            )
            ...
            if len(args) <= 2:
                ...
                i = localtime()[6] * 3
                print(
                    f"The current date is: {'MonTueWedThuFriSatSun'[i : i + 3]} {localtime()[1]:02}/{localtime()[2]:02}/{localtime()[0]:04}"
                )
            else:
            ...
            if len(args) <= 2:
                if len(args) == 2:
                    if clockavail:
                        ...
                        print("Real Time Clock (rtc) not available on board")
                print(
                    f"The current time is: {localtime()[3] % 12}:{localtime()[4]:02}:{localtime()[5]:02} {['AM', 'PM'][localtime()[3] // 12]}"
                )
            else:
            ...
            if len(args) < 3:
                ...
                if args[i].strip().upper() == "ERRORLEVEL":
                    ...
                    if len(args) > i and args[i].isdigit():
                        if str(envVars.get("errorlevel")).isdigit() and int(
                            envVars.get("errorlevel")
                        ) == int(args[i]):
                            condResult = True
            ...
            if len(args) == 1:
                ...
                if len(switches) <= 1:
                    if len(switches) == 0:
                        ...
                        print(
                            "Illegal switch, Command Format: SET[/a|/p] [variable = [string|expression]]"
                        )
                ...
                if validPath:
                    if not (swAllB - int("000010", 2)) & swBits:
                        if "*" in newdir or "?" in newdir:
                            ...
                            if newdir == "*" or newdir == "*.*":
                                ans = input(
                                    tmpDir + newdir + ", Are you sure (y/n)? "
                                ).upper()
                            ...
                            if newdir in os.listdir(pFmt(tmpDir, False)):
                                ...
                                if swBits & int("000010", 2):
                                    ...
                                    print(
                                        f"Unable to delete: {tmpDir}{newdir}. File not found."
                                    )
                    else:
            ...
            if len(args) == 2:
                if (
                    validPath
                    and newdir in os.listdir(pFmt(tmpDir, False))
                    and aFile(tmpDir + newdir)
                ):
                    if cmd == "MORE":
            ...
            if (len(args) == 3 or len(args) == 2) and not swBits & (
                swAllB - int("00001000", 2)
            ):
                if len(args) == 2:
                    args.append(".")
                ...
                if (
                    ...
                    and (
                        ...
                        or "?" in newdir
                        or (
                            newdir in os.listdir(tmpDir)
                            and aFile(pFmt(tmpDir) + newdir)
                        )
                    )
                ):
                    sourcePath = tmpDir
                    ...
                    if validPath:
                        ...
                        if newdir2 in "..":
                            (validPath, tmpDir) = chkPath(
                                (pFmt(targetPath) + newdir2).split(sep)
                            )
                        ...
                        if newdir2 in os.listdir(targetPath) or (
                            ...
                            if aFile(pFmt(targetPath) + newdir2):
                                if trailingSlash:
                                    print(
                                        "Cannot find the specified path: ", enteredArg
                                    )
                                elif wildCardOp:
                                    print(
                                        "Target must be directory for wildcard copy ",
                                        enteredArg,
                                    )
                                    ...
                                    swBits & int("001000", 2)
                                    or input(
                                        "Overwrite " + args[2] + "? (y/n): "
                                    ).upper()
                                    == "Y"
                                ):
                                    os.remove(pFmt(targetPath) + newdir2)
                                    filecpy(
                                        pFmt(sourcePath) + newdir,
                                        pFmt(targetPath) + newdir2,
                                    )
                                    nFiles += 1
                            ...
                            # Second argument target is a directory
                                ...
                                if wildCardOp:
                                    ...
                                    for _dir in os.listdir(pFmt(sourcePath, False)):
                                        if aFile(sourcePath + _dir) and _match(
                                            newdir, _dir[:wldCLen]
                                        ):
                                            print(
                                                "copy",
                                    ...
                                    if sourcePath == pFmt(targetPath + newdir2):
                                        ...
                                        filecpy(
                                            sourcePath + newdir,
                                            pFmt(targetPath + newdir2) + newdir,
                                        )
                                        nFiles += 1
                ...
                if swBits & (swAllB - int("001000", 2)):
                    print(
                        "Illegal switch, Command Format: COPY[/y] [path]file [path][file]"
                    )
                else:
                    print("Wrong number of arguments")
            ...
            if "xcopy" in aPath:
                envVars["_switches"] = [
                    i.upper() for i in aPath[aPath.index("xcopy") + 1 :]
                ]
