...
class EdlinEditor:
    ...
    def parse_command(self, cmd):
        ...
        if self.extract_range(cmd):
            ...
            elif cmd_without_range.startswith("?r") or cmd_without_range.startswith(
                "?s"
            ):
    ...
    def check_save(self):
        if self.modified:
            ...
            while save not in ["y", "yes", "n", "no"]:
                save = (
                    input("Text modified. Save before quitting? (y/n): ")
                    .lower()
                    .strip()
                )
            if save[0] == "y":
    ...
    def show_help(self):
        ...
        print(
            "If a line number or range is not specified, the current line will be used"
        )
    ...
    def process_range(self, parsed_cmd):
        ...
        if cmd == "s":
            ...
            if len(parts) < 3:
                print(
                    "Error: Need exactly 3 separators for replace command - Usage: [range] r/old/new/"
                )
                return
            ...
            if remaining.strip():
                print(
                    "Error: Extra characters after third separator - Usage: [range] r/old/new/"
                )
                return
                ...
                if search_text in self.lines[i - 1]:
                    if confirm:
                        ...
                        while response not in ["y", "yes", "n", "no", "e", "end"]:
                            if cmd == "s":
                                response = (
                                    input(f"Continue search? (Yes/No): ")
                                    .lower()
                                    .strip()
                                )
                            else:
                                response = (
                                    input(
                                        f"Replace '{search_text}' with '{new_text}'? (Yes/No/End): "
                                    )
                                    .lower()
                                    .strip()
                                )
                        ...
                        if response[0] == "n":
                            ...
                            if cmd == "r":
                                self.lines[i - 1] = self.lines[i - 1].replace(
        ...
        if cmd in ["s", "r"]:
            if _count > 0:
                print(
                    f"{'Found' if cmd == 's' else 'Replaced'} '{search_text}' {
                        "with '" if cmd == 'r' else ''
                    }{new_text if cmd == 'r' else ''}{"'" if cmd == 'r' else ''}in {
                        _count
                    } line(s)"
                )
            else:
    ...
    def _open_text(self, name):
        ...
        if (
            self._chkPath(aPath)
            and newdir in os.listdir(name[0 : len(name) - len(newdir)])
            and os.stat(name)[0] & (2**15) != 0
        ):
    ...
    def open_file(self, filename=None):
        ...
        if self.modified:
            ...
            while save not in ["y", "yes", "n", "no"]:
                save = (
                    input(
                        "Current file is modified. Save before opening new file? (y/n): "
                    )
                    .lower()
                    .strip()
                )
            if save[0] == "y":
                if self.filename:
                    ...
                    temp_filename = input(
                        "Enter filename to save current file: "
                    ).strip()
                    if temp_filename:
                        ...
                        while sure not in ["y", "yes", "n", "no"]:
                            sure = input(
                                "Current changes will be lost! Are you Sure? (y/n): "
                            )
                        if sure[0] == "n":
